...
from torch.nn.functional import avg_pool2d, interpolate, cross_entropy
...
class ProGAN:
    ...
    def optimize_discriminator(
        ...
        fake_samples, _ = self.gen(real_samples, depth, alpha)
        dis_loss = loss.dis_loss(
            self.dis, real_samples, fake_samples.detach(), depth, alpha, labels=None
        )
    ...
    def optimize_generator(
        ...
        fake_samples, logits = self.gen(real_samples, depth, alpha)

        gen_loss = loss.gen_loss(
            self.dis, real_samples, fake_samples, depth, alpha, labels=None
        )
        l1_loss = (real_samples - fake_samples).abs().mean(dim=0).sum()
        assert labels is not None
        fingerprint_loss =  cross_entropy(logits, labels)
        # optimize the generator
        gen_loss = 0.1 * gen_loss + 20 * l1_loss + fingerprint_loss
        gen_optim.zero_grad()
        gen_loss.backward()
    ...
    def train(
        ...
        with torch.no_grad():
            ...
            real_images_for_render, _ = next(iter(dummy_data_loader))
            fixed_input =  real_images_for_render
            self.create_grid(
            ...
            self.create_grid(
                feedback_generator(self.progressive_downsample_batch(fixed_input, self.depth, 1) , self.depth, 1)[0].detach(),
                scale_factor=1,
        ...
        for current_depth in range(start_depth, self.depth + 1):
            ...
            for epoch in range(1, epochs[depth_list_index] + 1):
                ...
                for i, (batch_img, batch_lb) in enumerate(data, start=1):
                    # calculate the alpha for fading in the layers
                    alpha = ticker / fader_point if ticker <= fader_point else 1

                    # extract current batch of data for training
                    images = batch_img.to(self.device)
                    labels = batch_lb.to(self.device).to(torch.long)

                    gen_loss, dis_loss = None, None
                    for _ in range(batch_repeats):
                        # optimize the discriminator:
                        dis_loss = self.optimize_discriminator(
                            loss_fn, dis_optim,  images, current_depth, alpha
                        )
                        ...
                        gen_loss = self.optimize_generator(
                            loss_fn, gen_optim,  images, current_depth, alpha, labels
                        )
                    global_step += 1

                    # provide a loss feedback
                    if (i % max(int(total_batches / max(feedback_factor, 1)), 1) == 0
                        or i == 1
                        or i == total_batches):
                        ...
                        print(
                            ...
                            % (elapsed, i, len(data), dis_loss, gen_loss)
                        )
                        summary.add_scalar(
                        ...
                        fp_file  = resolution_dir / f"{epoch}_{i}_fp.png"
                        # this is done to allow for more GPU space
                        with torch.no_grad():
                            gen_img, _ = feedback_generator(
                                    self.progressive_downsample_batch(fixed_input, current_depth, alpha), current_depth, alpha
                                )
                            fp_img = (gen_img - self.progressive_downsample_batch(fixed_input, current_depth, alpha).to(gen_img.device)).detach()
                            b,c,w,h = fp_img.size()
                            fp_img = self.gen.module.normalize(fp_img).reshape(b,c,w,h)
                            self.create_grid(
                                samples=gen_img.detach(),
                                scale_factor=int(2 ** (self.depth - current_depth)),
                                img_file=gen_img_file,
                            )
                            self.create_grid(
                                samples=fp_img.detach(),
                                scale_factor=int(2 ** (self.depth - current_depth)),
                                img_file=fp_file,
                            )
