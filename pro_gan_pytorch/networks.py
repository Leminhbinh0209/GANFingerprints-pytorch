...
class EncoderDecoder(th.nn.Module):
    ...
    def __init__(
        ...
        self.encoder = Encoder(depth,  num_channels, latent_size, use_eql)
        self.decoder = Generator(depth,  num_channels, latent_size, use_eql, num_classes)
        # For finall classification
        self.fingerprint = Parameter(torch.FloatTensor(num_classes, num_channels*(2**self.depth) * (2**self.depth)))
        torch.nn.init.xavier_uniform_(self.fingerprint)
    ...
    def forward(
        ...
        Args:
            x: input to the network: B x C x H x W
        ...
        """
        embedding = self.encoder(x, depth, alpha, labels)
        # print("zero test:", embedding.shape)
        gen_img = self.decoder(embedding, depth, alpha )
        # print("First test: ", x.shape, gen_img.shape)
        img_vectorize = self.normalize(gen_img-x) 
        weight_vectorize = self.normalize(self.fingerprint)
        st_idx = (weight_vectorize.size(1) - img_vectorize.size(1)) // 2
        # print("Second test: ", st_idx, img_vectorize.size(), weight_vectorize.size())
        logits = F.linear(img_vectorize, weight_vectorize[:, st_idx:(st_idx+img_vectorize.size(1))])

        return gen_img, logits
...
class Encoder(th.nn.Module):
    ...
    def __init__(
        ...
        if self.conditional:
            ...
            self.layers = [EncFinalBlock(nf(1), latent_size, use_eql)]
