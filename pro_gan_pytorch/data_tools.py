...
class ImageDirectoryDataset(Dataset):
    ...
    def __init__(
        ...
        assert os.path.exists(data_dir), print(f"{data_dir} not exists")
        self.files = self._get_files(data_dir, rec_dir)
      
        classes = [ dir_path for dir_path in os.listdir(data_dir) if ( os.path.isdir(str(data_dir) + "/" + dir_path))]
        classes.sort()
    ...
    def _get_files(self, path: Path, rec: bool = False) -> List[Path]:
        ...
        for possible_file in path.iterdir():
            if possible_file.is_file():
                ...
                files.extend(self._get_files(possible_file, rec))
        return files
